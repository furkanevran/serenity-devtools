...
export function WidgetListItem({ widget }: { widget: WidgetInfo }) {
    ...
    const isSelected = selectedWidget?.uniqueName === widget.uniqueName;

    return (
        <div 
            data-unique-name={widget.uniqueName}
            className={`ms-1 cursor-pointer border-l-2 transition-colors border-blue-900 ${isSelected ? "bg-blue-600" : ""} ${widget.isVisible ? "border-green-500" : ""}`}>
